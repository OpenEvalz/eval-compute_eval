...
Evaluates LLM capability to generate correct CUDA code for kernel implementation, memory
management, and parallel algorithm optimization tasks.

## At a glance

| | |
|---|---|
| Upstream | [`src/inspect_evals/compute_eval`](https://github.com/UKGovernmentBEIS/inspect_evals/tree/main/src/inspect_evals/compute_eval) |
| Group | Coding |
| Total samples | 406 |
...
- `huggingface` — `nvidia/compute-eval` (pinned)
...
```bash
inspect eval inspect_evals/compute_eval \
  --sample-id "<sample-id>" \
